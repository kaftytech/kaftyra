...
trait HandlesPurchaseOrderSelection
{
    ...
    public function updatedOrderSearchByQuery()
    {
        ...
        $query = PurchaseOrder::select('id', 'po_number')->where('vendor_id', $this->selectedVendor);

        foreach ($searchWords as $word) {
            $query->where(function ($subquery) use ($word) {
                $subquery->where('po_number', 'like', '%' . $word . '%');
            });
        }
    ...
    public function selectHighlightedOrder()
    {
        if (isset($this->orders[$this->highlightIndexOrder])) {
            $this->selectPurchaseOrder($this->orders[$this->highlightIndexOrder]['id']);
        }
    }
    ...
    public function selectPurchaseOrder($purchaseOrderId)
    {
        ...
        $this->order_search_by_query = $purchaseOrder->order_id;
        $this->orders = [];  // Clear the customer list once selected
        $this->order_search_by_query = $purchaseOrder->po_number;
        $this->order_search_by_query_status = 'selected';
        $this->order_id = $purchaseOrder->id;
        $this->updatePurchaseOrderId($purchaseOrder->id);
    }
}
